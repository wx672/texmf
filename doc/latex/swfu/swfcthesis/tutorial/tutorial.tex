...
\newCJKfontfamily\cjkquotefont{KaiTi}
...
\item[31-33] 这三行用来选择中文字体，非要不可。SimSun就是Windows宋体； WenQuanYi Micro Hei是
  文泉驿微米黑，不错的开源字体；
\item[34-37] 这四行用于在文章中间变换字体，如果你没这个需求，就不必管它。
  举几个例子，
  \begin{itemize}
  \item \{\verb'\CJKfamily{kai}'楷体\} $\Rightarrow$ {\CJKfamily{kai}楷体}
  \item \{\verb'\CJKfontspec{LiSu}'隶书\} $\Rightarrow$
    {\CJKfontspec{LiSu}隶书}
  % \item \{\verb'\CJKfontspec{badashanren}'八大山人毛笔字体\} $\Rightarrow$
  %   {\CJKfontspec{badashanren}八大山人毛笔字体}
  \item \{\verb'\CJKfontspec{QXyingbixing}'全新硬笔行书简体\} $\Rightarrow$
    {\CJKfontspec{QXyingbixing}全新硬笔行书简体}
  \end{itemize}
